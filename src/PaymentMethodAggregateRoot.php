...
class PaymentMethodAggregateRoot implements AggregateRootWithSnapshotting
{
    ...
    {
        $command->getToken()->isExpired() && throw new TokenExpiredException();

        $self = new static($command->getId());
        $self->recordThat(new PaymentMethodCreated($command->getBillingAddress(), $command->getToken()->getCard(), $command->getToken()->aggregateRootId()));
